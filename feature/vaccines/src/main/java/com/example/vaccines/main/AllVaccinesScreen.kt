...
import com.example.ui_components.components.card.VaccineCard
...
@Composable
fun AllVaccinesScreen(
    uiState: AllVaccinesUiState,
    uiActions: AllVaccinesUiActions,
    ...
    LaunchedEffect(vaccines.loadState.refresh) {
        vaccines.let {
            ...
            if (uiState.screenState == ScreenState.SUCCESS) {
                PullToRefreshBox(
                    ...
                    LazyColumn(
                        ...
                        items(vaccines.itemCount) { index ->
                            val vaccine = vaccines[index]
                            when (vaccine) {
                                null -> Unit
                                else -> VaccineCard(
                                    vaccine = vaccine,
                                    onClick = {
                                        uiActions.navigateToVaccineDetailsScreen(vaccine.id ?: -1)
                                    },
                                    modifier = Modifier
                                        .fillMaxSize()
                                )
                        ...
                        if (vaccines.loadState.append == LoadState.Loading) {
                            item {
